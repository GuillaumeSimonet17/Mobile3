...
class _WeatherHomePageState extends State<WeatherHomePage>
          ...
          if (_dailyWeather.isNotEmpty)
            ...
            SizedBox(
              ...
              child: Padding(
                ...
                child: LineChart(_getTemperatureChart()),
              ),
            ),
            Expanded(
  ...
  LineChartData _getTemperatureChart() {
    return LineChartData(
      ...
      titlesData: FlTitlesData(
        leftTitles: AxisTitles(
          sideTitles: SideTitles(
            showTitles: true,
            interval: 5, // Affichage tous les 5°C
            reservedSize: 40,
          ),
        ),
      ...
      lineBarsData: [
        LineChartBarData(
          spots: _dailyWeather.asMap().entries.map((entry) {
            int index = entry.key;
            var hourData = entry.value;
            return FlSpot(index.toDouble(), hourData.temperature.toDouble());
          }).toList(),
          isCurved: true,
          color: Colors.blue,
        ...
        if (errorText.isNotEmpty)
          ...
          if (_weeklyWeather.isNotEmpty)
            ...
            SizedBox(
              ...
              child: Padding(
                padding: const EdgeInsets.all(8.0),
                child: LineChart(_getWeeklyTemperatureChart()),
              ),
            ),
  ...
  LineChartData _getWeeklyTemperatureChart() {
    return LineChartData(
        ...
        bottomTitles: AxisTitles(
          sideTitles: SideTitles(
            ...
            getTitlesWidget: (value, meta) {
              ...
              if (index >= 0 && index < _weeklyWeather.length) {
                return Padding(
                  padding: const EdgeInsets.only(top: 8.0),
                  child: Text(
                    DateFormat('EEEE').format(_weeklyWeather[index].time),
      ...
      lineBarsData: [
        ...
        LineChartBarData(
          spots: _weeklyWeather.asMap().entries.map((entry) {
            int index = entry.key;
            var dayData = entry.value;
            return FlSpot(index.toDouble(), dayData.temperatureMin.toDouble());
          }).toList(),
          isCurved: true,
          color: Colors.blue,
          dotData: FlDotData(show: true),
        ...
        LineChartBarData(
          spots: _weeklyWeather.asMap().entries.map((entry) {
            int index = entry.key;
            var dayData = entry.value;
            return FlSpot(index.toDouble(), dayData.temperatureMax.toDouble());
          }).toList(),
          isCurved: true,
          color: Colors.red,
          dotData: FlDotData(show: true),
